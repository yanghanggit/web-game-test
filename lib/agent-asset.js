...
const { listStructuredAssets } = require('./cocos-asset-finder');
...
function listAllImageAssets() {
  ...
  const structuredAssets = listStructuredAssets(ASSETS_ROOT);
  for (const asset of structuredAssets) {
    if (!asset.file || !fs.existsSync(asset.file)) continue;
    const currentName = seen.get(asset.file);
    if (!currentName || currentName === path.basename(asset.file)) {
      seen.set(asset.file, asset.name || asset.fullUUID || path.basename(asset.file));
    }
  }
